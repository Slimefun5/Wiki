...
Slimefun5 is a really big plugin and therefore always raises questions about its performance.<br>
...
This article should help you discover bottlenecks and limitations and guide you to optimize your server and your Slimefun5 configuration to run as smooth as possible.<br>
Here are some of the most important tips on how to optimize your Server or your Slimefun5 configuration:
...
### b) Slimefun5 Profiling (/sf timings)
Slimefun5 also provides its own profiling tool which allows you to find sources of lag.<br>
...
You will surely see some differences between the different content in Slimefun5 and any addons.
...
**Cargo networks** from Slimefun5 have been optimized to work best when using [Paper](https://papermc.io/).<br>
...
A lot of plugins are not meant to deal with reloads and Slimefun5 is one of them, you should always restart your server instead.

## 4. Disabling backwards-compatibility
Slimefun5 has been around for a long time and there have been many Servers who use it since years.<br>
Any Server that has used Slimefun5 **before summer 2019** will have a bunch of old Slimefun5 Items. 
...
Everytime a player clicks with an Item in their hand, Slimefun5 has to compare this item with every Slimefun5 item in existence, including any items from addons.
...
The new item format instead assigns any Slimefun5 item NBT tags that tell the plugin what item the player is actually holding.
...
You can optimize your Server with this method by setting `backwards-compatibility` to `false` in your `plugins/Slimefun5/config.yml`.
```yaml
options:
...
## 5. Slower Tick-rates
Many Slimefun5 blocks execute code on a very regular basis, the default for this setting is set to run these tasks every 10 MC ticks (20 MC ticks = 1 second).<br>
...
You can configure this setting in your `plugins/Slimefun5/config.yml`. We recommend to only make small changes to the default value of 10 MC ticks when necessary.
```yaml
URID:
  custom-ticker-delay: 10
```

Similar to this setting, Slimefun5 regularly checks Player's armor to apply effects that come with wearing specific sets of armor.
...
You can set a maximum network size for cargo- and energy networks in your `plugins/Slimefun5/config.yml`.<br>
...
Lastly, one of the most effective ways to optimize your performance is to keep automatic Slimefun5 updates enabled at all times!<br>
...
You should always use the latest version, so we highly recommend you to enable `auto-updates` in your `plugins/Slimefun5/config.yml`.
```yaml
options:
  auto-update: true
```
